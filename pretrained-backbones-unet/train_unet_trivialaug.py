...
train_transforms = torch.nn.Sequential(
    T.AutoAugment(fill=None)
#     T.TrivialAugmentWide(interpolation = torchvision.transforms.InterpolationMode.BILINEAR, fill=0)
#     T.RandAugment(fill=0)
)
...
model = Unet(backbone='resnet50', num_classes=1)

params = [p for p in model.parameters() if p.requires_grad]
# optimizer = torch.optim.AdamW(params, 1e-3) # IOU 0.6348, 500 epochs, DICE loss
optimizer = torch.optim.Adam(params, 1e-3)
# optimizer = torch.optim.SGD(params, lr=2e-4, momentum=0.9, nesterov=True)
# scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 3, T_mult=2)
# scaler = torch.cuda.amp.GradScaler()
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', patience=2, factor=0.7, cooldown=2)
...
#     scaler=scaler,
    ...
    save_dir = 'runs/unet_resnet50_autoaug_1',
    num_classes=2
)
